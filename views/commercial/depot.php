...
<div class="content-area overflow-y-auto p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Les opérations de dépot d'argents</h1>
    </div>

    <div class="overflow-hidden mb-6">
        <div class="overflow-x-auto">
            <form action="../../controllers/CommercialController.php" method="POST">
                <?php if (isset($_SESSION['erreur_depot'])) {
                    echo '<p class="text-red-500 pb-3">' . htmlspecialchars($_SESSION['erreur_depot']) . '</p>';
                    unset($_SESSION['erreur_depot']); 
                } ?>
                <?php if (isset($_SESSION['success_depot'])) {
                    echo '<p class="text-green-500 pb-3">' . htmlspecialchars($_SESSION['success_depot']) . '</p>';
                    unset($_SESSION['success_depot']); 
                } ?>
                <div class="grid grid-cols-2 gap-y-3 gap-x-8">
                    <input type="text" hidden name="action" value="depot">
                    <div>
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-100" name='montant'>Montant à déposer
                            (FCFA)</label>
